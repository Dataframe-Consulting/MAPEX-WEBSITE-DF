...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([cats, brands, product]) => {
      setCategories(cats.data || []);
      setBrands(brands.data || []);
      if (product.data) {
        const p = product.data as Product & { variants: ProductVariant[] };
        setForm({
          ...
          brand_id: p.brand_id || '', base_price: String(p.base_price), image_url: p.image_url || '',
          finish: p.finish || '', base_type: p.base_type || '',
          coverage_sqm_per_liter: p.coverage_sqm_per_liter ? String(p.coverage_sqm_per_liter) : '',
          dry_time_hours: p.dry_time_hours ? String(p.dry_time_hours) : '',
          coats_required: p.coats_required ? String(p.coats_required) : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('products').update({
      ...
      base_price: parseFloat(form.base_price), image_url: form.image_url || null,
      finish: form.finish || null, base_type: form.base_type || null,
    ...
    if (deletedVariants.length > 0) {
      await supabase.from('product_variants').delete().in('id', deletedVariants);
    }

    for (const v of variants.filter(v => v.name && v.price)) {
      if (v.id) {
        ...
        await supabase.from('product_variants').insert({
          product_id: id, name: v.name, price: parseFloat(v.price),
          compare_price: v.compare_price ? parseFloat(v.compare_price) : null,
          stock: parseInt(v.stock) || 0, sku: v.sku || null, is_active: true,
        });
      }
    }
  ...
  const addVariant = () => setVariants(p => [...p, { name: '', price: '', compare_price: '', stock: '0', sku: '' }]);
  const removeVariant = (i: number) => {
    const v = variants[i];
    if (v.id) setDeletedVariants(p => [...p, v.id!]);
  ...
  if (fetching) {
    return <div className="p-8 flex items-center justify-center min-h-96"><div className="h-8 w-8 border-2 border-[#E8462A] border-t-transparent rounded-full animate-spin" /></div>;
  }

  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-8">
        <Link href="/admin/productos" className="text-gray-500 hover:text-gray-700"><ArrowLeft className="h-5 w-5" /></Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Nombre del producto *" value={form.name}
                onChange={e => setForm(p => ({ ...p, name: e.target.value, slug: slugify(e.target.value) }))} required />
            ...
            <div>
              ...
              <input type="text" value={form.short_description} onChange={e => setForm(p => ({ ...p, short_description: e.target.value }))}
                className="w-full h-10 px-4 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#E8462A]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Descripción completa</label>
              <textarea rows={4} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#E8462A] resize-none" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
            ...
            <Input label="Precio base *" type="number" step="0.01" value={form.base_price}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              ...
              <select value={form.finish} onChange={e => setForm(p => ({ ...p, finish: e.target.value }))}
                ...
                {['flat','matte','eggshell','satin','semi-gloss','gloss','high-gloss'].map(f => <option key={f} value={f}>{f}</option>)}
              </select>
            </div>
            <div>
            ...
            <Input label="Rendimiento (m²/L)" type="number" step="0.1" value={form.coverage_sqm_per_liter}
              onChange={e => setForm(p => ({ ...p, coverage_sqm_per_liter: e.target.value }))} />
            <Input label="Secado (horas)" type="number" step="0.5" value={form.dry_time_hours}
              onChange={e => setForm(p => ({ ...p, dry_time_hours: e.target.value }))} />
          </div>
          <div className="mt-4">
            <Input label="Etiquetas (separadas por coma)" value={form.tags}
              onChange={e => setForm(p => ({ ...p, tags: e.target.value }))} />
          </div>
          <div className="mt-4 flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_featured} onChange={e => setForm(p => ({ ...p, is_featured: e.target.checked }))} className="w-4 h-4 accent-[#E8462A]" />
              <span className="text-sm font-medium text-gray-700">Producto destacado</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_active} onChange={e => setForm(p => ({ ...p, is_active: e.target.checked }))} className="w-4 h-4 accent-[#E8462A]" />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-black text-gray-900">Variantes</h2>
            <button type="button" onClick={addVariant} className="flex items-center gap-1.5 text-sm text-[#E8462A] hover:underline font-semibold">
          ...
          <div className="space-y-3">
            {variants.map((v, i) => (
              <div key={i} className="grid grid-cols-5 gap-3 items-end">
                <Input label={i === 0 ? 'Nombre' : ''} placeholder="1L, 4L, 19L" value={v.name} onChange={e => updateVariant(i, 'name', e.target.value)} />
                <Input label={i === 0 ? 'Precio *' : ''} type="number" step="0.01" placeholder="0.00" value={v.price} onChange={e => updateVariant(i, 'price', e.target.value)} />
                <Input label={i === 0 ? 'Precio original' : ''} type="number" step="0.01" placeholder="0.00" value={v.compare_price} onChange={e => updateVariant(i, 'compare_price', e.target.value)} />
                <Input label={i === 0 ? 'Stock' : ''} type="number" placeholder="0" value={v.stock} onChange={e => updateVariant(i, 'stock', e.target.value)} />
                <button type="button" onClick={() => removeVariant(i)}
                  className="h-10 w-10 flex items-center justify-center text-red-400 hover:bg-red-50 rounded-lg transition-colors self-end">
        ...
        <div className="flex items-center gap-3">
          <Button type="submit" variant="primary" size="lg" loading={loading}><Save className="h-4 w-4" /> Guardar cambios</Button>
          <Button type="button" variant="ghost" size="lg" asChild><Link href="/admin/productos">Cancelar</Link></Button>
        </div>
      </form>
    </div>
  );
